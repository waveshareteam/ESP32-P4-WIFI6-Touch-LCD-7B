...
static const char *TAG = "usb_extend_lvgl_v9";
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0)
    buf1 = heap_caps_calloc(1, buffer_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA | MALLOC_CAP_CACHE_ALIGNED);
#else
...
static void touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    esp_lcd_touch_point_data_t touchpad_data;
    uint8_t touchpad_cnt = 0;
    /* Read data from touch controller into memory */
    esp_lcd_touch_read_data(tp);

    /* Read data from touch controller */
    bool touchpad_pressed = esp_lcd_touch_get_data(tp, &touchpad_data, &touchpad_cnt, 1) == ESP_OK;
...
static void lvgl_port_task(void *arg)
{
    ...
    while (1) {
        usb_extend_lvgl_lock(0);

        uint32_t task_delay_ms = lv_timer_handler();

        usb_extend_lvgl_unlock();
...
esp_err_t usb_extend_lvgl_init(esp_lcd_panel_handle_t lcd_in, esp_lcd_touch_handle_t tp_in, lv_display_t **disp_out, lv_indev_t **indev_out)
{
    ...
    usb_extend_display_callback_t display_cb = {
#if LVGL_PORT_AVOID_TEAR_ENABLE
    ...
    usb_extend_display_register_callback(&display_cb);
...
bool usb_extend_lvgl_lock(uint32_t timeout_ms)
{
    assert(lvgl_mux && "usb_extend_lvgl_init must be called first");

    const TickType_t timeout_ticks = (timeout_ms == 0) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
    return xSemaphoreTakeRecursive(lvgl_mux, timeout_ticks) == pdTRUE;
}

void usb_extend_lvgl_unlock(void)
{
    assert(lvgl_mux && "usb_extend_lvgl_init must be called first");
