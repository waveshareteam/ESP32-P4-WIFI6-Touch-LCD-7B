...
static esp_err_t uac_device_output_cb(uint8_t *buf, size_t len, void *arg)
{
    ...
    usb_extend_audio_i2s_write(buf, len, &bytes_written, 0);
    return ESP_OK;
}

static esp_err_t uac_device_input_cb(uint8_t *buf, size_t len, size_t *bytes_read, void *arg)
{
    if (usb_extend_audio_i2s_read(buf, len, bytes_read, 0) != ESP_OK) {
...
static void uac_device_set_mute_cb(uint32_t mute, void *arg)
{
    ...
    usb_extend_audio_codec_mute_set(mute);
}

static void uac_device_set_volume_cb(uint32_t volume, void *arg)
{
    ESP_LOGD(TAG, "uac_device_set_volume_cb: %"PRIu32"", volume);
    usb_extend_audio_codec_volume_set(volume, NULL);
}

esp_err_t app_uac_init(void)
{
    usb_extend_audio_codec_init();
    usb_extend_audio_codec_set_fs(CONFIG_UAC_SAMPLE_RATE, 16, CONFIG_UAC_SPEAKER_CHANNEL_NUM);
