...
The exact `esp_lcd_ek79007` 2.0.2~1 pin provides the native IDF 6 MIPI/DMA2D
layout while retaining IDF >=5.3 metadata; the `esp-audio-player` 1.1.0 pin and
explicit split-driver dependencies keep this project's sources buildable.
GitHub Actions verifies compilation only; it does not verify hardware runtime.
It does not build the bundled firmware or ESP-Brookesia separately.
The EK79007 version-gated API plus the IDF 6 allocator/DMA2D path are compile
compatibility only and do not verify LCD runtime behavior.
