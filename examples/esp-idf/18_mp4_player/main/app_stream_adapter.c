...
#define EXTRACT_TASK_STOP_TIMEOUT_MS 2000
#define EXTRACT_TASK_POLL_INTERVAL_MS 20
...
typedef struct app_stream_adapter_t {
    ...
    SemaphoreHandle_t extract_state_mutex;    /*!< Mutex for task/join state */
    bool task_finished;                       /*!< Extraction task has left its loop */
    SemaphoreHandle_t operation_mutex;        /*!< Binary gate for lifecycle operations */
...
static esp_err_t validate_decode_buffers(void **decode_buffers,
                                         uint32_t buffer_count,
                                         uint32_t buffer_size)
{
    if (decode_buffers == NULL || buffer_count == 0 || buffer_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }

    for (uint32_t i = 0; i < buffer_count; i++) {
        if (decode_buffers[i] == NULL) {
            return ESP_ERR_INVALID_ARG;
        }
    }
...
static void extract_task(void *arg)
{
    ...
    /* Publish completion under the state lock. After releasing it, the task
     * must not touch adapter state or the event group again. */
    xSemaphoreTake(adapter->extract_state_mutex, portMAX_DELAY);
    adapter->extract_task_handle = NULL;
    adapter->task_finished = true;
    xSemaphoreGive(adapter->extract_state_mutex);
    vTaskDelete(NULL);
}
...
{
    if (xSemaphoreTake(adapter->extract_state_mutex, portMAX_DELAY) != pdTRUE) {
        return ESP_ERR_INVALID_STATE;
    }
...
// Stop extraction task and wait for its state publication.
static esp_err_t stop_extract_task(app_stream_adapter_t *adapter)
{
    if (adapter == NULL || adapter->extract_event_group == NULL ||
        adapter->extract_state_mutex == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (adapter->extract_task_handle == NULL) {
        const bool finished = adapter->task_finished;
        xSemaphoreGive(adapter->extract_state_mutex);
        if (finished) {
            xEventGroupClearBits(adapter->extract_event_group,
                                 EXTRACT_TASK_START_BIT | EXTRACT_TASK_STOP_BIT);
    ...
    const TickType_t timeout_ticks = pdMS_TO_TICKS(EXTRACT_TASK_STOP_TIMEOUT_MS);
    ...
    while (true) {
        ...
        if (finished) {
            /* The worker released the state lock before this observation and
             * cannot access the event group or adapter again. */
            xEventGroupClearBits(adapter->extract_event_group,
                                 EXTRACT_TASK_START_BIT | EXTRACT_TASK_STOP_BIT);
            return ESP_OK;
        }

        if ((TickType_t)(xTaskGetTickCount() - deadline) < (TickType_t)0x80000000U) {
            ESP_LOGW(TAG, "Extract task did not stop within timeout period");
            /* Keep EXTRACT_TASK_STOP_BIT set. The worker still owns adapter
             * resources and a later serialized stop may observe completion. */
            return ESP_ERR_TIMEOUT;
        }

        vTaskDelay(pdMS_TO_TICKS(EXTRACT_TASK_POLL_INTERVAL_MS));
    }
}
                                  ...
                                  app_stream_adapter_handle_t *ret_adapter)
{
    ...
    if (validate_decode_buffers(config->decode_buffers,
                                config->buffer_count,
                                config->buffer_size) != ESP_OK) {
        ESP_LOGE(TAG, "Invalid decode buffer configuration");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    adapter->extract_state_mutex = xSemaphoreCreateMutex();
    if (adapter->extract_state_mutex == NULL) {
        ESP_LOGE(TAG, "Failed to create extract state mutex");
    ...
    adapter->operation_mutex = xSemaphoreCreateBinary();
    if (adapter->operation_mutex == NULL) {
        ESP_LOGE(TAG, "Failed to create operation mutex");
        vSemaphoreDelete(adapter->extract_state_mutex);
        vEventGroupDelete(adapter->extract_event_group);
        vSemaphoreDelete(adapter->frame_mutex);
        heap_caps_free(adapter->jpeg_buffer);
        free(adapter);
...
static esp_err_t lock_operation(app_stream_adapter_t *adapter)
{
    if (adapter == NULL || adapter->operation_mutex == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    if (xSemaphoreTake(adapter->operation_mutex, 0) != pdTRUE) {
        ESP_LOGW(TAG, "Concurrent stream operation rejected");
        return ESP_ERR_INVALID_STATE;
    }
    return ESP_OK;
}

static bool extract_task_is_active(app_stream_adapter_t *adapter)
{
    bool active;
    xSemaphoreTake(adapter->extract_state_mutex, portMAX_DELAY);
    active = (adapter->extract_task_handle != NULL);
    xSemaphoreGive(adapter->extract_state_mutex);
    return active;
}

static esp_err_t stop_locked(app_stream_adapter_t *adapter)
{
                                      ...
                                      bool extract_audio)
{
    ...
    esp_err_t ret = lock_operation(adapter);
    if (ret != ESP_OK) {
        return ret;
    }

    ret = stop_locked(adapter);
    if (ret != ESP_OK) {
        xSemaphoreGive(adapter->operation_mutex);
        return ret;
    }
...
esp_err_t app_stream_adapter_seek(app_stream_adapter_handle_t handle, uint32_t position)
{
    ...
    bool was_running = adapter->running || extract_task_is_active(adapter);
    if (was_running) {
    ...
    if (was_running) {
        esp_err_t restart_ret = start_extract_task(adapter);
...
esp_err_t app_stream_adapter_deinit(app_stream_adapter_handle_t handle)
{
    ...
    if (adapter->extractor_handle != NULL) {
        ret = app_extractor_deinit(adapter->extractor_handle);
        if (ret != ESP_OK) {
            xSemaphoreGive(adapter->operation_mutex);
            return ret;
        }
                                            ...
                                            uint32_t buffer_size)
{
    ...
    if (validate_decode_buffers(decode_buffers, buffer_count, buffer_size) != ESP_OK) {
        ESP_LOGE(TAG, "Invalid decode buffer configuration");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (adapter->running || extract_task_is_active(adapter)) {
        xSemaphoreGive(adapter->operation_mutex);
        return ESP_ERR_INVALID_STATE;
    }
