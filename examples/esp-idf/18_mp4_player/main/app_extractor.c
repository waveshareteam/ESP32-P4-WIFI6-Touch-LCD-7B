...
static esp_err_t process_audio_frame(app_extractor_t *extractor, uint8_t *buffer, uint32_t buffer_size, uint32_t pts)
{
    ...
    if (extractor->audio_buffer == NULL) {
        extractor->audio_buffer_size = APP_AUDIO_BUFFER_SIZE;
...
/**
 * @brief Validate MP4/AVI format compatibility for ESP32-P4
 *
 * This function ensures that only hardware JPEG compatible formats are processed:
 * - Video: MJPEG only (ESP32-P4 hardware JPEG decoder limitation)
 * - Audio: AAC, MP3, PCM, or enabled optional decoders
 *
 * @param extractor App extractor handle
 * @return ESP_OK if compatible, ESP_ERR_NOT_SUPPORTED if incompatible
 */
static esp_err_t validate_media_compatibility(app_extractor_t *extractor)
{
...
#if CONFIG_MP4_PLAYER_VIDEO_SYNC_ENABLED
    extractor->sync_enabled = true;
    extractor->sync_threshold_ms = APP_SYNC_THRESHOLD_MS;
                              ...
                              bool extract_audio)
{
    ...
    ret = validate_media_compatibility(extractor);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Media compatibility validation failed: %d", ret);
        return ret;
    }
    ...
    ESP_LOGI(TAG, "Extraction started: frame_interval=%" PRIu32 " ms, audio=%s",
