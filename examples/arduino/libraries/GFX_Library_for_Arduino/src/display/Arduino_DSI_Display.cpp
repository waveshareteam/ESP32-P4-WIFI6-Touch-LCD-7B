...
bool Arduino_DSI_Display::begin(int32_t speed)
{
  ...
  {
    // The LCD-7B EK79007 panel uses the BSP's active-low reset sequence:
    // assert low for 10 ms, release high, then wait 20 ms before commands.
    pinMode(_rst, OUTPUT);
  ...
  if (!_dsipanel->begin(_fb_width, _fb_height, speed, _init_operations, _init_operations_len))
  {
    return false;
  }
