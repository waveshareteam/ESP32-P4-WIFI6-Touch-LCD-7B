...
def current_repository_files(root: Path) -> set[str]:
    """Return current files with the exact repository spelling of each path."""
    try:
        data = run_git(
            root,
        ...
        return {
            posix_path(path.relative_to(root))
            for path in root.rglob("*")
            if path.is_file()
        }
    return {
        posix_path(item)
        for item in data.decode("utf-8", errors="surrogateescape").split("\0")
        if item
    }
...
def bilingual_findings(
    ...
    checked_paths = {
        change.path
        for change in selected
        if change.status != "D" and exists(change.path)
    }
...
def audit(
    ...
    *,
    ...
    current_files = current_repository_files(root)
    ...
    candidates = [
        ...
        if change.status != "D" and change.path.lower().endswith(".md")
        and change.path in current_files and (root / change.path).is_file()
