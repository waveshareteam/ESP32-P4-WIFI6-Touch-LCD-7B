...
class MarkdownAuditTests(unittest.TestCase):
    ...
    def test_case_only_pair_move_uses_exact_current_repository_paths(self) -> None:
        original_is_file = Path.is_file

        def case_insensitive_alias(path: Path) -> bool:
            relative = path.as_posix()
            if relative.endswith("/Docs/guide.md") or relative.endswith("/Docs/guide_ZH.md"):
                return True
            return original_is_file(path)

        changes = [
            audit_markdown.Change("D", "Docs/guide.md"),
            audit_markdown.Change("D", "Docs/guide_ZH.md"),
            audit_markdown.Change("A", "docs/guide.md"),
            audit_markdown.Change("A", "docs/guide_ZH.md"),
        ]
        with mock.patch.object(Path, "is_file", case_insensitive_alias):
            report = self.report(changes)
        self.assertNotIn("BILINGUAL_PAIR_REMOVED", self.codes(report))
    ...
    def test_github_footnote_definition_is_not_a_reference_link(self) -> None:
        self.write(
            "docs/links.md",
            "[简体中文](links_ZH.md)\n\nA statement.[^1]\n\n"
            "[^1]: **NOTE:** This is explanatory text, not a link target.\n",
        )
        self.write("docs/links_ZH.md", "[English](links.md)\n")
        report = self.report(self.changes("docs/links.md", "docs/links_ZH.md"))
        self.assertNotIn("RELATIVE_LINK_MISSING", self.codes(report))
    ...
    def test_homepage_hero_and_product_link_contract_regressions(self) -> None:
        config = audit_markdown.load_config(None)
        config["homepage_pairs"] = [
            {
                ...
                "profile": "auto",
                "required_components": ["hero_image"],
                "required_quick_links": ["product"],
            }
        ]

        for path in ("README.md", "README_ZH.md"):
            text = (self.root / path).read_text(encoding="utf-8")
            self.write(path, re.sub(r"(?m)^  <img src=\"assets/hero\.png\".*\n", "", text))
        report = self.report(
        ...
        hero_findings = [
            ...
            if item["code"] == "HOMEPAGE_REQUIRED_COMPONENT_MISSING"
        ]
        self.assertEqual({"README.md", "README_ZH.md"}, {item["path"] for item in hero_findings})
        self.assertTrue(all("component 'hero_image'" in item["message"] for item in hero_findings))

        self.write("README.md", EN_HOME.replace(" · <a href=\"https://example.invalid/product\">🌐 Product Page</a>", ""))
        self.write("README_ZH.md", ZH_HOME.replace(" · <a href=\"https://zh.example.invalid/product\">🌐 产品页面</a>", ""))
        report = self.report(
            self.changes("README.md", "README_ZH.md", status="M"), config=config
        )
        product_findings = [
            item
            for item in report["findings"]
            if item["code"] == "HOMEPAGE_REQUIRED_QUICK_LINK_MISSING"
        ]
        self.assertEqual({"README.md", "README_ZH.md"}, {item["path"] for item in product_findings})
        self.assertTrue(all("['product']" in item["message"] for item in product_findings))

        self.write("README.md", EN_HOME.replace("🌐 Product Page", "Product Page"))
        self.write("README_ZH.md", ZH_HOME.replace("🌐 产品页面", "产品页面"))
        report = self.report(
            self.changes("README.md", "README_ZH.md", status="M"), config=config
        )
        icon_findings = [
            item
            for item in report["findings"]
            if item["code"] == "HOMEPAGE_QUICK_LINK_ICON"
        ]
        self.assertEqual({"README.md", "README_ZH.md"}, {item["path"] for item in icon_findings})
        self.assertTrue(all("semantic 'product'" in item["message"] for item in icon_findings))
...
"""
    ...
    def test_bundled_config_template_is_neutral_and_exposes_generic_settings(self) -> None:
        path = SKILL_ROOT / "assets" / "markdown-audit-config.json"
