...
static char st_wifi_ssid[WIFI_SSID_MAX_LENGTH + 1];
...
typedef enum {
    ...
    WIFI_EVENT_DISCONNECTED = BIT(4)
} wifi_event_id_t;

static esp_err_t copy_wifi_field(uint8_t *destination, size_t destination_size,
                                 const char *source, size_t max_length)
{
    ...
    if (length > max_length || length > destination_size) {
...
static esp_err_t disconnect_wifi_and_wait(void)
{
    ...
    xEventGroupClearBits(s_wifi_event_group, WIFI_EVENT_CONNECTED | WIFI_EVENT_DISCONNECTED);
    ...
    return (bits & WIFI_EVENT_DISCONNECTED) ? ESP_OK : ESP_ERR_TIMEOUT;
}
...
bool AppSettings::init(void)
{
    ...
    _nvs_param_map[NVS_KEY_AUDIO_VOLUME] = product_audio_codec_volume_get();
    ...
    product_audio_codec_volume_set(_nvs_param_map[NVS_KEY_AUDIO_VOLUME], (int *)&_nvs_param_map[NVS_KEY_AUDIO_VOLUME]);
    ...
    xEventGroupClearBits(s_wifi_event_group,
                         WIFI_EVENT_CONNECTED | WIFI_EVENT_INIT_DONE |
...
esp_err_t AppSettings::initWifi()
{
    ...
    xEventGroupClearBits(s_wifi_event_group,
                         WIFI_EVENT_CONNECTED | WIFI_EVENT_INIT_DONE |
                         WIFI_EVENT_SCANING | WIFI_EVENT_DISCONNECTED);
...
void AppSettings::resetWifiScanUi(void)
{
    ...
    if (!bsp_display_lock(-1)) {
        ESP_LOGW(TAG, "Failed to lock display while stopping Wi-Fi scan");
        return;
    }
    lv_obj_add_flag(ui_PanelScreenSettingWiFiList, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_SpinnerScreenSettingWiFi, LV_OBJ_FLAG_HIDDEN);
...
void AppSettings::scanWifiAndUpdateUi(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Wi-Fi scan result count failed: %s", esp_err_to_name(ret));
        resetWifiScanUi();
        return;
    }
    ...
    if (bsp_display_lock(-1)) {
        ...
        ESP_LOGW(TAG, "Failed to lock display while refreshing Wi-Fi list");
    }

    for (int i = 0; (i < SCAN_LIST_SIZE) && (i < ap_count); i++) {
        const bool psk_flag = ap_info[i].authmode != WIFI_AUTH_OPEN &&
                              ap_info[i].authmode != WIFI_AUTH_OWE;
#if ENABLE_DEBUG_LOG
        ...
        if (bsp_display_lock(-1)) {
            ...
            ESP_LOGW(TAG, "Failed to lock display while adding Wi-Fi result");
        }
    }
}
...
void AppSettings::wifiConnectTask(void *arg)
{
    ...
    char candidate_ssid[WIFI_SSID_MAX_LENGTH + 1] = { 0 };

    if (app == nullptr) {
        ESP_LOGE(TAG, "Wi-Fi connect task received a NULL app");
        vTaskDelete(NULL);
        return;
    }

    esp_err_t ret = disconnect_wifi_and_wait();
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Wi-Fi disconnect before connect failed: %s", esp_err_to_name(ret));
    }

    if (ret == ESP_OK) {
        if (app->_is_ui_del || !bsp_display_lock(1000)) {
            ret = ESP_ERR_INVALID_STATE;
        } else {
            const char *ssid_text = lv_label_get_text(ui_LabelScreenSettingVerificationSSID);
            const char *password_text = lv_textarea_get_text(ui_TextAreaScreenSettingVerificationPassword);
            ret = copy_wifi_field(wifi_config.sta.ssid, sizeof(wifi_config.sta.ssid),
                                  ssid_text, WIFI_SSID_MAX_LENGTH);
            if (ret == ESP_OK) {
                ret = copy_wifi_field((uint8_t *)candidate_ssid, sizeof(candidate_ssid),
                                      ssid_text, WIFI_SSID_MAX_LENGTH);
            }
            if (ret == ESP_OK) {
                ret = copy_wifi_field(wifi_config.sta.password, sizeof(wifi_config.sta.password),
                                      password_text, WIFI_PASSWORD_MAX_LENGTH);
            }
            bsp_display_unlock();
        }
    }
    ...
    if (ret == ESP_OK) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Wi-Fi set config failed: %s", esp_err_to_name(ret));
        }
    }
    ...
    if (ret == ESP_OK && (bits & WIFI_EVENT_CONNECTED)) {
        const esp_err_t record_ret = copy_wifi_field((uint8_t *)st_wifi_ssid, sizeof(st_wifi_ssid),
                                                     candidate_ssid, WIFI_SSID_MAX_LENGTH);
        if (record_ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to record connected Wi-Fi SSID: %s", esp_err_to_name(record_ret));
        }
        ESP_LOGI(TAG, "Connected successfully");

        if (!app->_is_ui_del && bsp_display_lock(-1)) {
...
void AppSettings::wifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED) {
        xEventGroupClearBits(s_wifi_event_group, WIFI_EVENT_DISCONNECTED);
...
void AppSettings::onSwitchPanelScreenSettingWiFiSwitchValueChangeEventCallback( lv_event_t * e) {
    ...
    if (state & LV_STATE_CHECKED) {
        ...
        if (app->_screen_index == UI_WIFI_SCAN_INDEX) {
            app->stopWifiScan();
            if (xEventGroupGetBits(s_wifi_event_group) & WIFI_EVENT_CONNECTED) {
                const esp_err_t ret = disconnect_wifi_and_wait();
                if (ret != ESP_OK) {
                    ESP_LOGW(TAG, "Wi-Fi disconnect request failed: %s", esp_err_to_name(ret));
                }
                app->status_bar->setWifiIconState(0);
            }
        }
...
void AppSettings::onButtonWifiListClickedEventCallback(lv_event_t * e)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Wi-Fi scan stop failed: %s", esp_err_to_name(ret));
    }
}
...
void AppSettings::onSliderPanelVolumeSwitchValueChangeEventCallback( lv_event_t * e) {
    ...
    if (volume != app->_nvs_param_map[NVS_KEY_AUDIO_VOLUME]) {
        if ((product_audio_codec_volume_set(volume, NULL) != ESP_OK) && (product_audio_codec_volume_get() != volume)) {
