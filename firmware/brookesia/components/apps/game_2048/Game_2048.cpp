...
bool Game2048::init(void)
{
    ...
    if (product_audio_player_init() != ESP_OK) {
        ESP_LOGE(TAG, "Play init with SPIFFS failed");
        return false;
    }

    if (product_audio_file_instance_init(BSP_SPIFFS_MOUNT_POINT MUSIC_DIR, &_file_iterator) != ESP_OK) {
        ESP_LOGE(TAG, "product_audio_file_instance_init failed");
        return false;
    }
