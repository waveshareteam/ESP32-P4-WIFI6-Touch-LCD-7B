...
static void stop_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        ESP_ERROR_CHECK(bsp_display_lock(-1) ? ESP_OK : ESP_ERR_INVALID_STATE);
    }
}
...
static void show_video_task(void *arg)
{
    ...
    if ((player_ctx.bgm_path != NULL) && product_audio_player_play_file(player_ctx.bgm_path) != ESP_OK) {
...
err:
    ...
    if (player_ctx.bgm_path != NULL) {
        product_audio_player_register_callback(NULL, NULL);
...
void esp_lvgl_simple_player_resume(void)
{
    ...
    if (player_ctx.state == PLAYER_STATE_PAUSED) {
        ...
        player_ctx.state = PLAYER_STATE_PLAYING;
        if (player_ctx.bgm_path != NULL) {
            if (!product_audio_player_is_playing_by_path(player_ctx.bgm_path)) {
                ESP_LOGW(TAG, "BGM is not playing. Start playing.");
                if (product_audio_player_play_file(player_ctx.bgm_path) != ESP_OK) {
                    ESP_LOGE(TAG, "Play bgm failed");
                }
